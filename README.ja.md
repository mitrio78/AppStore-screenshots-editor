...
ローカル dev サーバーを起動してブラウザーでエディターを開きます。

停止するには、`start.command` が開いたターミナルで `Ctrl+C` を押します。

### Windows

File Explorer で `start.cmd` をダブルクリックします。初回起動時に依存関係をインストールし、
ローカル dev サーバーを起動して既定のブラウザーでエディターを開きます。

停止するには、`start.cmd` が開いたコマンドウィンドウで `Ctrl+C` を押します。
